...
<div class="flex flex-wrap items-end justify-between gap-4">
    <div>
        <h3 class="font-display text-lg font-bold text-ink-900">Material {{ $technology->code }}</h3>
    ...
    <div class="flex flex-wrap gap-2">
        ...
        <a href="{{ route('superadmin.price-list.materials.create', $technology) }}" class="btn-primary">Tambah Material {{ $technology->code }}</a>
    </div>
</div>
...
<form method="POST" id="{{ $tab }}-bulk-delete"
      ...
      data-bulk-form="{{ $tab }}"
      data-bulk-noun="material {{ $technology->code }}">
    @csrf
    @method('DELETE')
</form>
...
<div class="mt-4 hidden flex-wrap items-center justify-between gap-3 rounded-2xl border border-brand-200 bg-brand-50 px-4 py-3"
    ...
    <p class="text-sm font-semibold text-brand-700">
        <span data-bulk-count="{{ $tab }}">0</span> material {{ $technology->code }} dipilih
    </p>
                ...
                <tr class="border-b border-ink-100 bg-ink-50/80 text-[0.65rem] uppercase tracking-[0.14em] text-ink-500">
                    <th scope="col" class="w-10 px-4 py-4">
                        <input type="checkbox"
                               ...
                               data-bulk-all="{{ $tab }}"
                               aria-label="Pilih seluruh material {{ $technology->code }} di halaman ini">
                    </th>
                ...
                @empty
                    <tr>
                        <td colspan="11" class="px-4 py-12 text-center text-ink-400">
                            Belum ada material {{ $technology->code }}.
